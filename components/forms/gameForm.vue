<template>
	<div class="my-form-style">
		<FormKit v-model="state" type="form" :config="{ validationVisibility: 'live' }" submit-label="Submit Game"
			...
			<div v-if="(reset_opponent && !addForm) || addForm">
				<Card style="width: 25em; margin-bottom: 1rem">
					...
					<template #content>
						<AutoComplete v-model="selectedOpponent" optionLabel="opponent_name" :suggestions="filteredOpponents"
							@complete="search_opponents" @item-select="setOpponent" />
						<p class="m-2">
							If the opponent for this game can not be found in the existing
							opponents list, you must first create the opponent<br />
							<Button class="p-button-sm mt-1" label="Create Opponent" @click="navigateTo('/admin/opponents/add')">
							</Button>
						</p>
					</template>
				</Card>
			...
			<FormKit label="Opponent" name="opponent_name" type="text" disabled="true" />
			...
			<!-- Game Type input-->
			<FormKit type="select" label="Game type" placeholder="Select game type" name="game_type_id" :options="gametypes"
				validation="required" />
			<!-- Game Level input-->
			<FormKit type="select" label="Game level" placeholder="Select game level" name="game_level" :options="[
				{ label: 'A', value: 'A' },
				{ label: 'B', value: 'B' },
				{ label: 'C', value: 'C' },
				{ label: 'D', value: 'D' },
	...
	">
		...
		<div class="my-form-style">
			...
			<div v-if="state.date_ut">
				Check here <br />
				Uncheck to Cancel
				<input id="reset" v-model="reset" type="checkbox" @input="resetPlayers()" />
				<p style="color: red; padding: 2rem">
					Selecting another roster will replace existing players on this roster!
				</p>
				<div v-if="reset">
					<FormKit v-model="previous_game_id" type="select" label="Select another roster"
						placeholder="Select another roster" :options="previousgames" />
				</div>
			</div>
		</div>
	</div>
	...
	<div class="xs:w-full md:w-8 lg:w-6 mx-auto mt-4 my-text-style">
		<table v-if="players" style="border-collapse: collapse;
				border-spacing: 0;
				white-space: nowrap;
				overflow-x: auto;
			">
			...
			<tbody>
				<tr v-for="(item, index) in players" :key="item.position_id">
					...
					<td>
						<AutoComplete v-model="selectedPlayers[index]" :pt="{
							input: {
								class:
									'w-6rem md:w-8rem lg:w-10rem text-xs md:text-sm h-1rem md:h-2rem',
							},
						}" optionLabel="title" :suggestions="filteredNames" @complete="search" />
					</td>
					<td>
					...
					<td>
						<AutoComplete v-model="selectedReplacements[index]" optionLabel="title" :pt="{
							input: {
								class:
									'w-6rem md:w-8rem lg:w-10rem text-xs md:text-sm h-1rem md:h-2rem',
							},
						}" :suggestions="filteredNames" @complete="search" />
					</td>
				</tr>
			</tbody>
		</table>
	...
	<!-- Confirm deletion -->
	<Dialog v-model:visible="showReplaceDialog" :style="{ width: '450px' }" header="Confirm replacement" :modal="true">
		<div class="confirmation-content">
			<i class="pi pi-exclamation-triangle mr-3" style="font-size: 2rem" />
			<span class="text-red-600">Are you sure you want to replace this roster?</span>
		</div>

		<template #footer>
			<Button label="No" icon="pi pi-times" class="p-button-text" @click="showReplaceDialog = false" />
			<Button label="Yes" icon="pi pi-check" class="p-button-text" @click="confirmedReplace(previous_game_id)" />
		</template>
	</Dialog>
</template>
